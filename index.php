...
header('Content-Type: text/html; charset=UTF-8');
  ...
  $user = 'u47566';
  $pass = '8241937';
  $db = new PDO('mysql:host=localhost;dbname=u47566', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
  ...
  if(!empty($_COOKIE['hack'])){
    $messages[]='<div class="error">Вас пытались взломать! Авторизуйтесь снова и не переходите по подозрительным ссылкам</div>';
  ...
  session_start() && !empty($_SESSION['login'])) {
    ...
    try{
      ...
      $values['super'] = [];
      $sth = $db->prepare("SELECT * FROM Superpowers7 WHERE id = ?");
      $sth->execute(array($user_id));
      $user_data = ($sth->fetchAll(PDO::FETCH_COLUMN, 1)); 

      foreach ($user_data as $key=>$val){
        $values['super'][$val] = $val;
      }
    } 
...
if (empty($_POST['name'])) {
    ...
    setcookie('name_error', htmlspecialchars($_POST['name']), time() + 24 * 60 * 60);
    $errors = TRUE;
  }
  else {
    // Сохраняем ранее введенное в форму значение на месяц.
    setcookie('name_value', htmlspecialchars($_POST['name']), time() + 30 * 24 * 60 * 60);
  }
  // EMAIL
  if (empty($_POST['email'])){
    ...
    setcookie('email_error', htmlspecialchars($_POST['email']), time() + 24 * 60 * 60);
    $errors = TRUE;
  }
  else {
  ...
  if (empty($_POST['bio'])) {
    ...
    setcookie('bio_value', htmlspecialchars($_POST['bio']), time() + 30 * 24 * 60 * 60);
  }
  ...
  if(!checkToken($token)&&!empty($_COOKIE[session_name()])){
    ...
    setcookie('kon_value', '', 100000);
    setcookie('super_value', '', 100000);
    setcookie('bio_value', '', 100000);
    setcookie('contr_check_value', '', 100000);
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    try {
      $stmt = $db->prepare("SELECT id FROM users7 WHERE login =?");
    ...
    try {
      $stmt = $db->prepare("INSERT INTO application7 SET name = ?, email = ?, year = ?, gender = ?, kon = ?, bio = ?");//, login = ?, password = ?
      $stmt -> execute(array(
          $_POST['name'],
          $_POST['email'],
          $_POST['year'],
