...
<?php
function getToken(){ // сам токен не меняется, меняется его хэш при каждом вызове формы 
...
<body>
	...
	<?php 
		if (!empty($messages)) {
			if(isset($messages['hack'])){ print('<div id="messages"'); print($messages['hack_message']);}
			else if(isset($messages['save'])) print('<div id="messages" class="ok">'); else print('<div id="messages">');
	...
	<a href="admin.php">Войти в панель администратора</a>
		<form action="" method="POST">
			...
			<textarea id="bio" name="bio" <?php if(!empty($errors['bio']))  print 'class="error"';?> <?php if(empty($errors['bio'])&&!empty($values['bio'])) print 'class="ok"';?>><?php isset($_COOKIE['bio_error']) ? print trim($_COOKIE['bio_error']) : print htmlspecialchars($values['bio']) ?></textarea>
			...
			<input type="hidden" name="token" value="<?php print(getToken());?>">
